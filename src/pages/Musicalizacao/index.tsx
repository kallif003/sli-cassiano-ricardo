...
export async function getStaticProps() {
	...
	const morningTeacherData = await client.getAllByType(
		"morning_music_teacher_profile",
		{}
	)

	const afternoonTeacherData = await client.getAllByType(
		"afternoon_music_teacher_profile",
		{}
	)

	const mormingTeacher: Teachers[] = morningTeacherData.map(
		(e: any) => ({
			slug: e.uid,
	...
	const afternoonTeacher: Teachers[] = afternoonTeacherData.map(
		(e: any) => ({
			slug: e.uid,
			teacherName: e.data.teacher_name,
			teacherImg: e.data.teacher_img.url,
